...
<div style="background-color: #cce7ec; height: 390px; padding-top: 20px; padding-left: 20px;">
    <form action="<?php echo base_url('update/pedoman/' . $data->id) ?>" method="post" enctype="multipart/form-data" onsubmit="return validateForm()">
        <div style="display:flex; justify-content: start; gap: 20px; margin-bottom:20px; align-items: center;">
            <label for="nama">Nama Pedoman:</label>
            <input type="text" name="nama" value="<?php echo $data->nama ?>" style="border-radius: 5px; width: 300px; height: 40px;">
        </div>

        <div style="display:flex; justify-content: start; gap: 20px; align-items: center;">
            <label for="file_pdf" style="align-items: center;"> Buku Pedoman:</label>
            <label class="file-picker">

                <input type="file" value="<?php echo base_url('public/file_pedoman/') . $data->filenya ?>" name="filenya" accept="application/pdf" onchange="previewFile(this)">
